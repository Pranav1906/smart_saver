...
// Video download endpoint with yt-dlp (generic video link parser)
app.post('/download/video', async (req, res) => {
    try {
        console.log('Video download request received:', req.body);
        
        const { url, quality = 'best', type = 'video', cookiesTxtBase64 } = req.body;
        ...
        // Normalize video URLs to standard format
        const normalizeVideoUrl = (u) => {
            try {
                let workingUrl = u.trim();
                // Normalize short links and video URLs
                if (workingUrl.includes('youtu.be/')) {
                    const id = workingUrl.split('youtu.be/')[1].split(/[?&#]/)[0];
                    if (id) return `https://www.youtube.com/watch?v=${id}`;
                }
                // Normalize shorts URLs
                if (workingUrl.includes('/shorts/')) {
                    const id = workingUrl.split('/shorts/')[1].split(/[?&#]/)[0];
                    if (id) return `https://www.youtube.com/watch?v=${id}`;
                }
                return workingUrl;
            } catch (e) {
        ...
        // If cookies are provided (base64 Netscape format), write them to a temp file
        let cookiesArg = '';
        let tempCookiesPath = '';
        try {
            if (cookiesTxtBase64 && typeof cookiesTxtBase64 === 'string' && cookiesTxtBase64.trim().length > 0) {
                const cookiesBuffer = Buffer.from(cookiesTxtBase64, 'base64');
                tempCookiesPath = path.join(DOWNLOADS_DIR, `cookies_${uuidv4()}.txt`);
                fs.writeFileSync(tempCookiesPath, cookiesBuffer);
                cookiesArg = ` --cookies "${tempCookiesPath}"`;
                console.log('Using provided cookies file for this request');
            }
        } catch (cookieWriteErr) {
            console.error('Failed to write cookies file:', cookieWriteErr?.message || cookieWriteErr);
        }

        // Attempt multiple client/UA/header combinations before failing
        const countries = ['US', 'IN', 'SG'];
        const attempts = [];
        for (const country of countries) {
            // Desktop UA + android client
            attempts.push(`/opt/venv/bin/yt-dlp --force-ipv4 --geo-bypass --geo-bypass-country=${country} --no-playlist --merge-output-format mp4 --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" --add-header "Accept-Language:en-US,en;q=0.9" --add-header "Accept-Encoding:gzip, deflate, br" --add-header "DNT:1" --add-header "Connection:keep-alive" --add-header "Upgrade-Insecure-Requests:1" --add-header "Referer:https://www.youtube.com/" --add-header "Origin:https://www.youtube.com"${cookiesArg} -f "bv*[ext=mp4][height<=1080]+ba[ext=m4a]/b[ext=mp4][height<=1080]/best[height<=1080]" -o "${outputPath}" "${normalizedUrl}"`);
            // Android mobile UA
            attempts.push(`/opt/venv/bin/yt-dlp --force-ipv4 --geo-bypass --geo-bypass-country=${country} --no-playlist --merge-output-format mp4 --user-agent "Mozilla/5.0 (Linux; Android 10; SM-G975F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" --add-header "Referer:https://m.youtube.com/" --add-header "Origin:https://m.youtube.com"${cookiesArg} -f "best[ext=mp4][height<=720]/best[height<=720]" -o "${outputPath}" "${normalizedUrl}"`);
            // iOS client
            attempts.push(`/opt/venv/bin/yt-dlp --force-ipv4 --geo-bypass --geo-bypass-country=${country} --no-playlist --merge-output-format mp4 --user-agent "Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Mobile/15E148 Safari/604.1" --no-check-certificates --extractor-args "youtube:player_client=ios" --add-header "Referer:https://www.youtube.com/" --add-header "Origin:https://www.youtube.com"${cookiesArg} -f "best[ext=mp4][height<=1080]/best" -o "${outputPath}" "${normalizedUrl}"`);
            // Web client with innertube host hints
            attempts.push(`/opt/venv/bin/yt-dlp --force-ipv4 --geo-bypass --geo-bypass-country=${country} --no-playlist --merge-output-format mp4 --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=web,innertube_host=m.youtube.com" --add-header "Referer:https://www.youtube.com/" --add-header "Origin:https://www.youtube.com"${cookiesArg} -f "best[ext=mp4][height<=1080]/best" -o "${outputPath}" "${normalizedUrl}"`);
            // Android TV client
            attempts.push(`/opt/venv/bin/yt-dlp --force-ipv4 --geo-bypass --geo-bypass-country=${country} --no-playlist --merge-output-format mp4 --user-agent "Mozilla/5.0 (Linux; Android 10; BRAVIA 4K UR2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android_tv" --add-header "Referer:https://www.youtube.com/" --add-header "Origin:https://www.youtube.com"${cookiesArg} -f "best[ext=mp4][height<=1080]/best" -o "${outputPath}" "${normalizedUrl}"`);
        }
        // Minimal attempt at the end
        attempts.push(`/opt/venv/bin/yt-dlp --force-ipv4 --no-playlist --merge-output-format mp4${cookiesArg} -o "${outputPath}" "${normalizedUrl}"`);

        let lastError = null;
        for (const cmd of attempts) {
            try {
                console.log('Executing yt-dlp attempt:', cmd);
                ...
                lastError = new Error('File was not created after download');
            } catch (err) {
                console.error('Attempt failed:', err?.stderr || err?.message || err);
                lastError = err;
                // If YouTube explicitly asks to sign in, do not continue spamming requests
                const text = (err?.message || '') + '\n' + (err?.stderr || '');
                if (text.includes("Sign in to confirm you're not a bot") || text.includes('account to confirm')) {
                    break;
                }
            }
        }
        ...
        if (tempCookiesPath) {
            try { fs.unlinkSync(tempCookiesPath); } catch (_) {}
        }

        // If all attempts failed, map to friendly errors
        const errorMessage = lastError?.message || '';
        const stderr = lastError?.stderr || '';
        const normalizeText = (t) => (t || '').replace(/[\u2018\u2019]/g, "'").replace(/[\u201C\u201D]/g, '"');
        const nErr = normalizeText(errorMessage);
        const nStderr = normalizeText(stderr);

        const authPhrases = [
            "Sign in to confirm you're not a bot",
            'confirm your age',
            'age-restricted',
            'This video requires an account to view'
        ];
        const isAuthGate = authPhrases.some(p => nErr.includes(p) || nStderr.includes(p));
        if (isAuthGate) {
            res.status(400).json({
                success: false,
                error: 'Video requires authentication. This usually happens with age-restricted or private content.',
                details: 'The video may be age-restricted, private, or require login to access',
                suggestion: 'Provide cookiesTxtBase64 (Netscape cookies.txt) in the request body from a logged-in browser session to download restricted videos.'
            });
        } else if (nErr.includes('Failed to extract any player response') || nStderr.includes('Failed to extract any player response')) {
            res.status(400).json({
                success: false,
                error: 'Unable to extract video information. This may be due to recent changes in the video platform\'s API.',
                details: 'The video may be unavailable, private, or the platform has changed their API',
        ...
        console.error('Error in video download endpoint:', error);
        res.status(500).json({
            success: false,
        ...
        try {
            ...
            if (errorMessage.includes('There is no video in this post') || 
                ...
                try {
                    ...
                    const fallbackCommand = `/opt/venv/bin/yt-dlp --user-agent "Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Mobile/15E148 Safari/604.1" --no-check-certificates --extractor-args "instagram:login_required=false" --sleep-interval 2 --max-sleep-interval 5 -f "best" -o "${outputPath}" "${url}"`;
                    ...
                    const stderr = fallbackError.stderr || '';
...
app.post('/download/facebook', async (req, res) => {
    try {
        ...
        const command = `/opt/venv/bin/yt-dlp --no-check-certificates --extractor-args "facebook:login_required=false" --sleep-interval 2 --max-sleep-interval 5 -o "${outputPath}" "${url}"`;
        ...
        if (fs.existsSync(outputPath)) {
            ...
            if (stderr && stderr.includes('does not contain a video')) {
                ...
                const fallbackCommand = `/opt/venv/bin/yt-dlp --no-check-certificates --extractor-args "facebook:login_required=false" --sleep-interval 2 --max-sleep-interval 5 -f "best" -o "${outputPath}" "${url}"`;
...
try {
    server = app.listen(PORT, HOST, () => {
        ...
        console.log('  POST /test/youtube - Test video accessibility');
        console.log('  POST /test/instagram-error - Test Instagram error handling');
        console.log('  POST /download/video - Download video (generic link parser)');
