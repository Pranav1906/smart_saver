...
// --- In-memory job store for async downloads ---
const jobs = new Map(); // jobId -> { id, status: 'pending'|'running'|'completed'|'failed', filename, fileUrl, error }
...
function normalizeYouTubeUrl(inputUrl) {
    try {
        ...
        if (urlStr.includes('youtube.com/shorts/')) {
            const id = urlStr.split('youtube.com/shorts/')[1]?.split(/[?&#]/)[0];
...
function buildYtDlpCommand(baseUrl, opts = {}) {
    const {
        outputPath,
        format = 'b/bv*[height<=720]+ba/b[height<=720]',
        playerClient = 'web,web_creator',
    ...
    const headerFlags = headers.map(h => `--add-header "${h}"`).join(' ');
    const extra = Array.isArray(extraFlags) && extraFlags.length ? (' ' + extraFlags.join(' ')) : '';
    const baseFlags = `/usr/local/bin/yt-dlp --force-ipv4 --no-check-certificates --no-playlist -N 4 --user-agent "${userAgent}" --extractor-args "youtube:player_client=${playerClient}" ${headerFlags}${extra}`;

    if (getUrlsOnly) {
        return `${baseFlags} -g -f "${format}" "${baseUrl}"`;
    }

    // If caller passed a plain path ending with .mp4, still allow yt-dlp to pick
    // the actual container by appending .%(ext)s to improve reliability.
    const outputTemplate = outputPath && outputPath.includes('%(')
        ? outputPath
        : (String(outputPath || '').endsWith('.mp4')
            ? outputPath.replace(/\.mp4$/i, '.%(ext)s')
            : outputPath);
    const mergeFlag = '--merge-output-format mp4';
    return `${baseFlags} ${mergeFlag} -f "${format}" -o "${outputTemplate}" "${baseUrl}"`;
}
...
function findCreatedFileByPrefix(prefix) {
    try {
...
// Resolve direct media URL(s) for YouTube quickly (avoids long-running downloads)
app.post('/resolve/youtube', async (req, res) => {
    try {
        const { url } = req.body || {};
        if (!url) {
        ...
        try {
            ...
            return res.status(500).json({ success: false, error: 'yt-dlp not available' });
        }

        // Build fast commands with strict timeouts and minimal retries
        const fastOptsWeb = { getUrlsOnly: true, playerClient: 'web,web_creator', extraFlags: ['--extractor-args', 'youtube:webpage_download_webpage=True'] };
        const fastOptsAndroid = { getUrlsOnly: true, playerClient: 'android,player_skip=web', extraFlags: [] };
        const wrapCmd = (opts) => {
            const base = buildYtDlpCommand(normalizedUrl, opts);
            // Harden with fewer retries and shorter socket timeout
            return base + ' --no-warnings --no-call-home --socket-timeout 5 --retries 1 -R 1';
        };

        const execFast = (cmd) => execAsync(cmd, { timeout: 20000 })
            .then(({ stdout }) => stdout.split(/\r?\n/).map(s => s.trim()).filter(Boolean));

        try {
            const [result] = await Promise.race([
                execFast(wrapCmd(fastOptsWeb)).then(lines => [{ client: 'web', lines }]),
                execFast(wrapCmd(fastOptsAndroid)).then(lines => [{ client: 'android', lines }]),
                new Promise((_, reject) => setTimeout(() => reject(new Error('race-timeout')), 15000)),
            ]);
            if (result && result[0] && result[0].lines.length > 0) {
                return res.json({ success: true, urls: result[0].lines, normalizedUrl, client: result[0].client });
            }
        } catch (raceErr) {
            // fall through to slower single attempts
        }

        // Slow path attempts (still capped) one after another
        try {
            const linesWeb = await execFast(wrapCmd(fastOptsWeb));
            if (linesWeb.length > 0) return res.json({ success: true, urls: linesWeb, normalizedUrl, client: 'web' });
        } catch (_) {}
        try {
            const linesAndroid = await execFast(wrapCmd(fastOptsAndroid));
            if (linesAndroid.length > 0) return res.json({ success: true, urls: linesAndroid, normalizedUrl, client: 'android' });
        } catch (e) {}

        return res.status(504).json({ success: false, error: 'Resolver timeout or failure' });
    } catch (error) {
        console.error('Error in /resolve/youtube:', error);
        res.status(500).json({ success: false, error: 'Resolver failed', details: error.message?.slice(0, 500) });
    }
});

// Async job APIs to avoid edge timeouts
app.post('/jobs/youtube', async (req, res) => {
    try {
        const { url } = req.body || {};
        ...
        (async () => {
            try {
                ...
                const baseName = `youtube_${id}`;
                const plannedFileMp4 = `${baseName}.mp4`;
                const outputPath = path.join(DOWNLOADS_DIR, plannedFileMp4);
                // Prefer android client for Shorts reliability; try progressive formats first
                const cmd = buildYtDlpCommand(normalizedUrl, { 
                    outputPath, 
                    playerClient: 'android,player_skip=web,player_query=contentCheckOk=yes,playableInEmbed=yes', 
                    format: '22/18/bv*[height<=720]+ba/b[height<=720]',
                    extraFlags: ['--no-warnings','--no-cache-dir','--geo-bypass']
                });
                try {
                    await execAsync(cmd, { timeout: 180000 });
                } catch (e1) {
                    // Fallback 1: strict progressive 18
                    const cmd18 = buildYtDlpCommand(normalizedUrl, {
                        outputPath,
                        playerClient: 'android,player_skip=web,player_query=contentCheckOk=yes,playableInEmbed=yes',
                        format: '18',
                        extraFlags: ['--no-warnings','--no-cache-dir','--geo-bypass']
                    ...
                    try {
                        await execAsync(cmd18, { timeout: 180000 });
                    } catch (e2) {
                        // Fallback 2: best single file
                        const cmdb = buildYtDlpCommand(normalizedUrl, {
                            outputPath,
                            playerClient: 'android,player_skip=web,player_query=contentCheckOk=yes,playableInEmbed=yes',
                            format: 'b',
                            extraFlags: ['--no-warnings','--no-cache-dir','--geo-bypass']
                        ...
                        try {
                            await execAsync(cmdb, { timeout: 180000 });
                        } catch (e3) {
                            // Fallback 3: try web client progressive
                            const cmdWeb = buildYtDlpCommand(normalizedUrl, {
                                outputPath,
                                playerClient: 'web,web_creator',
                                format: '22/18',
                                extraFlags: ['--no-warnings','--no-cache-dir','--geo-bypass']
                            ...
                            try {
                                await execAsync(cmdWeb, { timeout: 180000 });
                            } catch (e4) {
                                // Ultimate fallback: let yt-dlp decide everything (no flags)
                                const defaultCmd = `/usr/local/bin/yt-dlp -o "${outputPath}" "${normalizedUrl}"`;
                                await execAsync(defaultCmd, { timeout: 180000 });
                            }
                        }
                    }
                }
                let finalPath = fs.existsSync(outputPath) ? outputPath : findCreatedFileByPrefix(baseName);
                if (!finalPath) throw new Error('File not created');
                const filename = path.basename(finalPath);
                const base = PUBLIC_BASE_URL;
                const fileUrl = base ? `${base}/file/${filename}` : null;
                jobs.set(id, { id, status: 'completed', filename, fileUrl });
            } catch (e) {
                jobs.set(id, { id, status: 'failed', error: (e && e.message) || 'unknown' });
            }
        })();
    } catch (error) {
...
app.get('/jobs/:id', (req, res) => {
    ...
    // If fileUrl wasn't set at completion time, synthesize from current host
    if (job.status === 'completed' && !job.fileUrl && job.filename) {
        job.fileUrl = `${req.protocol}://${req.get('host')}/file/${job.filename}`;
    }
...
app.post('/test/youtube', async (req, res) => {
    try {
        ...
        const testCommand = `/usr/local/bin/yt-dlp --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" --add-header "Accept-Language:en-US,en;q=0.9" --add-header "Accept-Encoding:gzip, deflate, br" --add-header "DNT:1" --add-header "Connection:keep-alive" --add-header "Upgrade-Insecure-Requests:1" --get-title --get-duration "${url}"`;
...
app.post('/download/youtube', async (req, res) => {
    try {
        ...
        const baseName = `youtube_${uuidv4()}`;
        const plannedFileMp4 = `${baseName}.mp4`;
        const outputPath = path.join(DOWNLOADS_DIR, plannedFileMp4);
        ...
        // Normalize Shorts URLs and prefer web clients; constrain format for speed
        const normalizedUrl = normalizeYouTubeUrl(url);
        const ytdlpCommand = buildYtDlpCommand(normalizedUrl, { outputPath });
        ...
        try {
            ...
            if (fs.existsSync(outputPath)) {
                ...
                console.log(`Download completed: ${plannedFileMp4} (${fileSize} MB)`);
                
                const fileUrl = `${req.protocol}://${req.get('host')}/file/${plannedFileMp4}`;
                
                res.json({
                    success: true,
                    message: 'Download completed successfully',
                    fileUrl: fileUrl,
                    filename: plannedFileMp4,
                    fileSize: `${fileSize} MB`,
                    originalUrl: url
                });
            } else {
                // Try to locate actual file with different extension
                const detectedPath = findCreatedFileByPrefix(baseName);
                if (detectedPath && fs.existsSync(detectedPath)) {
                    ...
                    return res.json({ success: true, message: 'Download completed', fileUrl, filename, fileSize: `${fileSize} MB`, originalUrl: url });
                }
            ...
            try {
                const fallbackCommand = buildYtDlpCommand(normalizedUrl, { outputPath, format: 'b/bv*+ba/best' });
                ...
                if (fs.existsSync(outputPath)) {
                    ...
                    console.log(`Fallback download completed: ${plannedFileMp4} (${fileSize} MB)`);
                    ...
                    if (detectedPath && fs.existsSync(detectedPath)) {
                        ...
                        const filename = path.basename(detectedPath);
                        const fileUrl = `${req.protocol}://${req.get('host')}/file/${filename}`;
                        return res.json({ success: true, message: 'Download completed (fallback format)', fileUrl, filename, fileSize: `${fileSize} MB`, originalUrl: url });
                    }
                ...
                try {
                    ...
                    const thirdFallbackCommand = buildYtDlpCommand(normalizedUrl, { outputPath, playerClient: 'android', format: 'b/worst' });
                    ...
                    if (fs.existsSync(outputPath)) {
                        ...
                        console.log(`Third fallback download completed: ${plannedFileMp4} (${fileSize} MB)`);
                        ...
                        if (detectedPath && fs.existsSync(detectedPath)) {
                            ...
                            const filename = path.basename(detectedPath);
                            const fileUrl = `${req.protocol}://${req.get('host')}/file/${filename}`;
                            return res.json({ success: true, message: 'Download completed (mobile fallback format)', fileUrl, filename, fileSize: `${fileSize} MB`, originalUrl: url });
                        }
                    ...
                    try {
                        ...
                        const fourthFallbackCommand = buildYtDlpCommand(normalizedUrl, { outputPath, format: 'b' });
                        ...
                        if (fs.existsSync(outputPath)) {
                            ...
                            console.log(`Fourth fallback download completed: ${plannedFileMp4} (${fileSize} MB)`);
                            ...
                            if (detectedPath && fs.existsSync(detectedPath)) {
                                ...
                                const filename = path.basename(detectedPath);
                                const fileUrl = `${req.protocol}://${req.get('host')}/file/${filename}`;
                                return res.json({ success: true, message: 'Download completed (web client fallback)', fileUrl, filename, fileSize: `${fileSize} MB`, originalUrl: url });
                            }
                        ...
                        try {
                            ...
                            const fifthFallbackCommand = buildYtDlpCommand(normalizedUrl, { outputPath, format: 'bv*[height<=480]+ba/b[height<=480]' });
                            ...
                            if (fs.existsSync(outputPath)) {
                                ...
                                console.log(`Fifth fallback download completed: ${plannedFileMp4} (${fileSize} MB)`);
                                ...
                                if (detectedPath && fs.existsSync(detectedPath)) {
                                    ...
                                    const filename = path.basename(detectedPath);
                                    const fileUrl = `${req.protocol}://${req.get('host')}/file/${filename}`;
                                    return res.json({ success: true, message: 'Download completed (minimal options fallback)', fileUrl, filename, fileSize: `${fileSize} MB`, originalUrl: url });
                                }
                            ...
                            try {
                                ...
                                const sixthFallbackCommand = buildYtDlpCommand(normalizedUrl, { outputPath, playerClient: 'web' });
                                ...
                                if (fs.existsSync(outputPath)) {
                                    ...
                                    console.log(`Sixth fallback download completed: ${plannedFileMp4} (${fileSize} MB)`);
                                    ...
                                    if (detectedPath && fs.existsSync(detectedPath)) {
                                        ...
                                        const filename = path.basename(detectedPath);
                                        const fileUrl = `${req.protocol}://${req.get('host')}/file/${filename}`;
                                        return res.json({ success: true, message: 'Download completed (default yt-dlp behavior)', fileUrl, filename, fileSize: `${fileSize} MB`, originalUrl: url });
                                    }
...
app.post('/download/instagram', async (req, res) => {
    try {
        ...
        // yt-dlp command for Instagram with enhanced anti-rate-limit measures
        const ytdlpCommand = `/usr/local/bin/yt-dlp --user-agent "Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Mobile/15E148 Safari/604.1" --add-header "Accept-Language:en-US,en;q=0.9" --add-header "Accept-Encoding:gzip, deflate, br" --add-header "DNT:1" --add-header "Connection:keep-alive" --add-header "Upgrade-Insecure-Requests:1" --add-header "Sec-Fetch-Dest:document" --add-header "Sec-Fetch-Mode:navigate" --add-header "Sec-Fetch-Site:none" --add-header "Sec-Fetch-User:?1" --no-check-certificates --extractor-args "instagram:login_required=false" --sleep-interval 2 --max-sleep-interval 5 -o "${outputPath}" "${url}"`;
        ...
        try {
            ...
            if (errorMessage.includes('There is no video in this post') || 
                ...
                try {
                    ...
                    const fallbackCommand = `/usr/local/bin/yt-dlp --user-agent "Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Mobile/15E148 Safari/604.1" --add-header "Accept-Language:en-US,en;q=0.9" --add-header "Accept-Encoding:gzip, deflate, br" --add-header "DNT:1" --add-header "Connection:keep-alive" --add-header "Upgrade-Insecure-Requests:1" --add-header "Sec-Fetch-Dest:document" --add-header "Sec-Fetch-Mode:navigate" --add-header "Sec-Fetch-Site:none" --add-header "Sec-Fetch-User:?1" --no-check-certificates --extractor-args "instagram:login_required=false" --sleep-interval 2 --max-sleep-interval 5 -f "best" -o "${outputPath}" "${url}"`;
                    const { stdout, stderr } = await execAsync(fallbackCommand, { timeout: 120000 });
                    ...
                    // Try third fallback with different approach
                    try {
                        console.log('Trying third fallback with embed approach...');
                        const thirdFallbackCommand = `/usr/local/bin/yt-dlp --user-agent "Mozilla/5.0 (iPad; CPU OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/16.0 Mobile/15E148 Safari/604.1" --add-header "Accept-Language:en-US,en;q=0.9" --add-header "Accept-Encoding:gzip, deflate, br" --add-header "DNT:1" --add-header "Connection:keep-alive" --add-header "Upgrade-Insecure-Requests:1" --no-check-certificates --extractor-args "instagram:login_required=false" --sleep-interval 3 --max-sleep-interval 7 --format "worst" -o "${outputPath}" "${url}"`;
                        const { stdout, stderr } = await execAsync(thirdFallbackCommand, { timeout: 120000 });
                        ...
                        if (fs.existsSync(outputPath)) {
                            ...
                            console.log(`Third fallback Instagram download completed: ${filename} (${fileSize} MB)`);
                            
                            const fileUrl = `${req.protocol}://${req.get('host')}/file/${filename}`;
                            
                            res.json({
                                success: true,
                                message: 'Instagram download completed successfully (third fallback)',
                                fileUrl: fileUrl,
                                filename: filename,
                                fileSize: `${fileSize} MB`,
                                originalUrl: url
                        ...
                        // Check for specific Instagram errors
                        const errorMessage = thirdFallbackError.message || '';
                        const stderr = thirdFallbackError.stderr || '';
                        
                        if (errorMessage.includes('rate-limit') || stderr.includes('rate-limit')) {
                            res.status(429).json({
                                success: false,
                                error: 'Instagram is temporarily blocking downloads due to rate limiting. Please try again in a few minutes.',
                                details: 'Rate limit reached - try again later',
                                retryAfter: 300 // 5 minutes
                            });
                        } else if (errorMessage.includes('login required') || stderr.includes('login required')) {
                            res.status(400).json({
                                success: false,
                                error: 'This Instagram content requires authentication. Please try with a public post.',
                                details: 'Private or restricted content cannot be downloaded'
                            });
                        } else {
                            res.status(500).json({
                                ...
                                details: thirdFallbackError.message
                            });
                        }
                    }
                }
...
// Instagram download via GET (debug/fallback): accepts ?url=
app.get('/download/instagram', async (req, res) => {
    try {
        ...
        // Delegate to POST handler logic by faking req.body
        req.body = { url };
        return app._router.handle({ ...req, method: 'POST', url: '/download/instagram' }, res, () => {});
    } catch (error) {
        console.error('GET /download/instagram error:', error);
        res.status(500).json({ success: false, error: error.message || 'Internal error' });
    }
});
...
app.post('/download/facebook', async (req, res) => {
    try {
        console.log('Facebook download request received:', req.body);
        ...
        try {
            await execAsync('/usr/local/bin/yt-dlp --version');
            console.log('yt-dlp is available for Facebook download');
        } catch (error) {
            console.error('yt-dlp not available for Facebook download:', error);
            return res.status(500).json({
                success: false,
        ...
        const filename = `facebook_${uuidv4()}.mp4`;
        const outputPath = path.join(DOWNLOADS_DIR, filename);
        
        console.log(`Starting Facebook download for: ${url}`);
        
        // yt-dlp command for Facebook with enhanced anti-rate-limit measures
        const command = `/usr/local/bin/yt-dlp --no-check-certificates --extractor-args "facebook:login_required=false" --sleep-interval 2 --max-sleep-interval 5 -o "${outputPath}" "${url}"`;
        
        console.log('Executing Facebook download command:', command);
        
        const { stdout, stderr } = await execAsync(command);
        
        console.log('Facebook download stdout:', stdout);
        if (stderr) console.log('Facebook download stderr:', stderr);
        ...
        if (fs.existsSync(outputPath)) {
            ...
            console.log(`Facebook download completed: ${filename} (${fileSize} MB)`);
            
            const fileUrl = `${req.protocol}://${req.get('host')}/file/${filename}`;
            
            res.json({
                success: true,
                message: 'Facebook download completed successfully',
                filename: filename,
                fileUrl: fileUrl,
                fileSize: `${fileSize} MB`
            });
            ...
            if (stderr && stderr.includes('does not contain a video')) {
                return res.status(400).json({
                    success: false,
                    error: 'This Facebook post does not contain a video. Please try with a post that has a video.'
                });
            } else if (stderr && (stderr.includes('unavailable') || stderr.includes('private'))) {
                return res.status(400).json({
                    success: false,
                    error: 'Video is private or unavailable. Try with a public post.'
                });
            } else if (stderr && (stderr.includes('temporarily blocking') || stderr.includes('rate limit'))) {
                return res.status(429).json({
                    success: false,
                    error: 'Facebook is temporarily blocking downloads. Please try again later.'
                });
            } else {
                // Try fallback download
                console.log('Trying fallback download for Facebook...');
                const fallbackCommand = `/usr/local/bin/yt-dlp --no-check-certificates --extractor-args "facebook:login_required=false" --sleep-interval 2 --max-sleep-interval 5 -f "best" -o "${outputPath}" "${url}"`;
                ...
                try {
                    ...
                    if (fs.existsSync(outputPath)) {
                        ...
                        console.log(`Fallback Facebook download completed: ${filename} (${fileSize} MB)`);
                        
                        const fileUrl = `${req.protocol}://${req.get('host')}/file/${filename}`;
                        
                        res.json({
                            success: true,
                            message: 'Facebook download completed successfully (fallback format)',
                            filename: filename,
                            fileUrl: fileUrl,
                            fileSize: `${fileSize} MB`
                        });
                    ...
                    console.error('Facebook fallback download failed:', fallbackError);
                    
                    if (fallbackError.stderr && fallbackError.stderr.includes('rate limiting')) {
                        return res.status(429).json({
                            success: false,
                            error: 'Facebook is temporarily blocking downloads due to rate limiting. Please try again in a few minutes.'
                        });
                    } else if (fallbackError.stderr && fallbackError.stderr.includes('authentication')) {
                        return res.status(401).json({
                            success: false,
                            error: 'This Facebook content requires authentication. Please try with a public post.'
                        });
                    } else {
                        return res.status(500).json({
                            success: false,
                            error: 'Failed to download Facebook content. Please check the URL and try again.'
                        });
                    }
                }
            }
        ...
        console.error('Error in Facebook download endpoint:', error);
        res.status(500).json({
            success: false,
            error: error.message || 'Internal server error'
        });
...
try {
    server = app.listen(PORT, HOST, () => {
        ...
        console.log('  POST /download/facebook - Download Facebook video');
