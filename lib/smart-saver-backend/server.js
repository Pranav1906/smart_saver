...
app.post('/resolve/youtube', async (req, res) => {
    try {
        ...
        try {
            const cmdWeb = buildYtDlpCommand(normalizedUrl, { getUrlsOnly: true, playerClient: 'web,web_creator' });
            const { stdout, stderr } = await execAsync(cmdWeb, { timeout: 60000 });
            const lines = stdout.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
            if (lines.length > 0) {
                return res.json({ success: true, urls: lines, normalizedUrl });
            }
        } catch (e) {
            // fall through to android client
        }

        // Fallback: Android client sometimes works better for Shorts
        try {
            const cmdAndroid = buildYtDlpCommand(normalizedUrl, { getUrlsOnly: true, playerClient: 'android' });
            const { stdout, stderr } = await execAsync(cmdAndroid, { timeout: 60000 });
            const lines = stdout.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
            if (lines.length > 0) {
                return res.json({ success: true, urls: lines, normalizedUrl, client: 'android' });
            }
            return res.status(502).json({ success: false, error: 'Failed to resolve media URLs', details: (stderr || '').slice(0, 500) || 'no output' });
        } catch (e) {
            const msg = (e && e.message) ? e.message : 'unknown error';
            return res.status(504).json({ success: false, error: 'Resolver timeout or failure', details: msg.slice(0, 500) });
        }
    } catch (error) {
