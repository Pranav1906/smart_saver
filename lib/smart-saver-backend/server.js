...
class VideoDownloader {
    ...
    async getVideoInfo(videoUrl) {
        try {
            ...
            if (this.isValidInstagramUrl(videoUrl)) {
                ...
                command = `yt-dlp --dump-json --user-agent "${this.userAgent}" --no-check-certificate "${cleanUrl}"`;
            } else {
                command = `yt-dlp --dump-json "${videoUrl}"`;
            }
    ...
    async installYtDlp() {
        try {
            ...
            const methods = [
                // Method 1: Using curl to download binary
                async () => {
                    ...
                    await execAsync('chmod +x /tmp/yt-dlp');
                    await execAsync('mv /tmp/yt-dlp /usr/local/bin/yt-dlp');
                },
                // Method 2: Using wget to download binary
                async () => {
                    console.log('Trying wget method...');
                    await execAsync('wget https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp -O /tmp/yt-dlp');
                    await execAsync('chmod +x /tmp/yt-dlp');
                    await execAsync('mv /tmp/yt-dlp /usr/local/bin/yt-dlp');
                },
