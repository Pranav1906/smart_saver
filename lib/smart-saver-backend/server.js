...
function buildYtDlpCommand(baseUrl, opts = {}) {
    ...
    return `${baseFlags} ${mergeFlag} -f "${format}" -o "${outputPath}" "${baseUrl}"`;
}
...
app.post('/resolve/youtube', async (req, res) => {
    try {
        ...
        const execFast = (cmd) => execAsync(cmd, { timeout: 15000 })
        ...
        try {
            const [result] = await Promise.race([
                ...
                new Promise((_, reject) => setTimeout(() => reject(new Error('race-timeout')), 11000)),
            ]);
    ...
    try {
        ...
        (async () => {
            try {
                ...
                const filename = `youtube_${id}.mp4`;
