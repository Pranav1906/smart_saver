...
app.post('/test/youtube', async (req, res) => {
    try {
        ...
        const testCommand = `yt-dlp --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" --get-title --get-duration "${url}"`;
...
app.post('/download/youtube', async (req, res) => {
    try {
        ...
        // yt-dlp command for YouTube Shorts with anti-bot measures
        const ytdlpCommand = `yt-dlp --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --cookies-from-browser chrome --no-check-certificates --extractor-args "youtube:player_client=android" -f "best[height<=1080]" -o "${outputPath}" "${url}"`;
        ...
        try {
            ...
            try {
                const fallbackCommand = `yt-dlp --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" -f "best" -o "${outputPath}" "${url}"`;
                ...
                try {
                    ...
                    const thirdFallbackCommand = `yt-dlp --user-agent "Mozilla/5.0 (Linux; Android 10; SM-G975F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" --format "worst" -o "${outputPath}" "${url}"`;
...
app.post('/download/instagram', async (req, res) => {
    try {
        ...
        const ytdlpCommand = `yt-dlp --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36" -o "${outputPath}" "${url}"`;
        ...
        try {
            ...
            if (errorMessage.includes('There is no video in this post') || 
                ...
                try {
                    ...
                    const fallbackCommand = `yt-dlp --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36" -f "best" -o "${outputPath}" "${url}"`;
                    const { stdout, stderr } = await execAsync(fallbackCommand, { timeout: 120000 });
