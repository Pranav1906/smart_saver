...
app.post('/download/youtube', async (req, res) => {
    try {
        ...
        const ytdlpCommand = `/opt/venv/bin/yt-dlp --force-ipv4 --geo-bypass --geo-bypass-country=US --no-playlist --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" --add-header "Accept-Language:en-US,en;q=0.9" --add-header "Accept-Encoding:gzip, deflate, br" --add-header "DNT:1" --add-header "Connection:keep-alive" --add-header "Upgrade-Insecure-Requests:1" -f "bv*[ext=mp4][height<=1080]+ba[ext=m4a]/b[ext=mp4][height<=1080]/best[height<=1080]" -o "${outputPath}" "${normalizedUrl}"`;
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    const thirdFallbackCommand = `/opt/venv/bin/yt-dlp --force-ipv4 --geo-bypass --no-playlist --user-agent "Mozilla/5.0 (Linux; Android 10; SM-G975F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" --add-header "Accept-Language:en-US,en;q=0.9" --add-header "Accept-Encoding:gzip, deflate, br" --add-header "DNT:1" --add-header "Connection:keep-alive" --add-header "Upgrade-Insecure-Requests:1" --format "bv*[ext=mp4][height<=720]+ba/b[height<=720]" -o "${outputPath}" "${normalizedUrl}"`;
                    ...
                    try {
                        ...
                        try {
                            ...
                            const fifthFallbackCommand = `/opt/venv/bin/yt-dlp --force-ipv4 --geo-bypass --no-playlist --user-agent "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=web" --format "bv*[ext=mp4][height<=720]+ba[ext=m4a]/b[ext=mp4][height<=720]" -o "${outputPath}" "${normalizedUrl}"`;
                            ...
                            try {
                                ...
                                const sixthFallbackCommand = `/opt/venv/bin/yt-dlp --force-ipv4 --no-playlist -o "${outputPath}" "${normalizedUrl}"`;
