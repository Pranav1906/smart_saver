...
function buildYtDlpCommand(baseUrl, opts = {}) {
    ...
    const baseFlags = `/usr/local/bin/yt-dlp --force-ipv4 --no-check-certificates --no-playlist -N 4 --user-agent "${userAgent}" --extractor-args "youtube:player_client=${playerClient}" ${headerFlags}`;
...
app.post('/resolve/youtube', async (req, res) => {
    try {
        ...
        const fastOptsWeb = { getUrlsOnly: true, playerClient: 'web,web_creator' };
        const fastOptsAndroid = { getUrlsOnly: true, playerClient: 'android' };
        const wrapCmd = (opts) => {
        ...
        const execFast = (cmd) => execAsync(cmd, { timeout: 10000 })
