    ...
    try {
        ...
        try {
            ...
            return res.status(500).json({
                success: false,
                error: 'Video downloader tool is not available. Please contact support.',
                details: 'yt-dlp not found in system PATH'
            });
        }
...
async function checkDependencies() {
    ...
    try {
        const { stdout } = await execAsync('yt-dlp --version');
        console.log(`✅ yt-dlp is available: ${stdout.trim()}`);
    } catch (error) {
        console.error('❌ yt-dlp is not available:', error.message);
        console.error('⚠️  Video downloads will not work without yt-dlp');
    }
    ...
    try {
        ...
        console.error('⚠️  Some video processing features may not work');
    }
