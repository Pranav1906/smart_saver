...
app.post('/resolve/youtube', async (req, res) => {
    try {
        ...
        // Build one very-fast command; no fallbacks, to guarantee sub-10s response
        const opts = { getUrlsOnly: true, playerClient: 'android,player_skip=web', extraFlags: [] };
        const base = buildYtDlpCommand(normalizedUrl, opts);
        const cmd = base + ' --no-warnings --no-call-home --socket-timeout 3 --retries 0 -R 0';

        try {
            const { stdout } = await execAsync(cmd, { timeout: 6000 });
            const lines = stdout.split(/\r?\n/).map(s => s.trim()).filter(Boolean);
            if (lines.length > 0) {
                return res.json({ success: true, urls: lines, normalizedUrl, client: 'android' });
            }
            return res.status(400).json({ success: false, error: 'No media URLs returned' });
        } catch (e) {
            return res.status(504).json({ success: false, error: 'Resolver timeout', details: (e && e.message) ? e.message.slice(0, 160) : '' });
        }
    } catch (error) {
