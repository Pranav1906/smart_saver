...
app.post('/test/youtube', async (req, res) => {
    try {
        ...
        const testCommand = `/opt/venv/bin/yt-dlp --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" --get-title --get-duration "${url}"`;
...
app.post('/download/youtube', async (req, res) => {
    try {
        ...
        const ytdlpCommand = `/opt/venv/bin/yt-dlp --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --cookies-from-browser chrome --no-check-certificates --extractor-args "youtube:player_client=android" -f "best[height<=1080]" -o "${outputPath}" "${url}"`;
        ...
        try {
            ...
            try {
                const fallbackCommand = `/opt/venv/bin/yt-dlp --user-agent "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" -f "best" -o "${outputPath}" "${url}"`;
                ...
                try {
                    ...
                    const thirdFallbackCommand = `/opt/venv/bin/yt-dlp --user-agent "Mozilla/5.0 (Linux; Android 10; SM-G975F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36" --no-check-certificates --extractor-args "youtube:player_client=android" --format "worst" -o "${outputPath}" "${url}"`;
